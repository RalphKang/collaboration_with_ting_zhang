...
def train_function(device,model, train_dl, optimizer, loss, ep, epoch, train_loss_record, sanity_check=False, lr_search=False):
    ...
    for i, (input_train, target_train) in enumerate(train_dl):
        input_train = input_train.squeeze().to(device)
        target_train =target_train.to(device)
        # forward calculation
        pred_train = model(input_train) 
        loss_train=loss(pred_train, target_train)
        abs=torch.abs(pred_train-target_train)
        error_number=torch.sum(abs>=0.5)
        accuracy=1-error_number/pred_train.shape[0]
        # loss_train = loss(out_train_hat, target_train)
        # loss_train=loss_train+pde_loss
        if lr_search:
    ...
    average_accuracy=accuracy_sum/total_batch_train
    print('train loss of the model: {:.6f},accuracy of the model: {:.6f},epoch={}'.format(average_loss_train,average_accuracy, ep + 1))
...
def vali_function(device,model, model_save_dir, vali_dl, loss, ep, epoch, vali_loss_record, sanity_check=False,
    ...
    with torch.no_grad():
        for j, (input_eval, target_eval) in enumerate(vali_dl):
            input_eval = input_eval.squeeze().to(device)
            target_eval = target_eval.to(device)
            pred_eval = model(input_eval)
            loss_eval=loss(pred_eval, target_eval)
            abs=torch.abs(pred_eval-target_eval)
            error_number=torch.sum(abs>=0.5)
            accuracy=1-error_number/pred_eval.shape[0]
        ...
        average_accuracy=accuracy_sum/total_batch_eval
        print('validation loss of the model: {:.6f},accuracy of the model: {:.6f},epoch={}'.format(average_loss,average_accuracy, ep + 1))
        vali_loss_record.append(average_loss)

    if average_accuracy.item() > historical_best:
        weight_best = copy.deepcopy(model.state_dict())
        torch.save(weight_best, model_save_dir)
        print("best model has been saved")
        historical_best=average_accuracy.item()
    return vali_loss_record,historical_best,average_accuracy
